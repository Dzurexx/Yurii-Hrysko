...
namespace MobileBank.Forms
{
    public partial class CommunalPayments : Form
    {
        ...
        private void CommunalPayments_Load(object sender, EventArgs e)
        {
            ...
            var queryChooseOperator = $"select id_services, serviceName from clientServices where serviceType = 'communal'";
            ...
            comboBoxCommunalPayment.DataSource = "serviceName";
        ...
        private void buttonPay_Click(object sender, EventArgs e)
        {
            ...
            if (cardCurrency != "PLN")
            {
                MessageBox.Show("Opłaty za media można uiścić wyłącznie w złotówkach", "Application Exit", MessageBoxButtons.OK, MessageBoxIcon.Information);
                error = true;
            }
            ...
            if (error == false)
            {
                ...
                if (DataStorage.attempts > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Opłata za usługi komunalne', '{comboBoxCommunalPayment.GetItemText(comboBoxCommunalPayment.SelectedItem)}'), '{transactionDate}', '{transactionNumber}', '{sum}', (select id_bank_card, from bank_card where bank_card_number = '{cardNumber}'))";

                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum}' where servisName = '{comboBoxCommunalPayment.GetItemText(comboBoxCommunalPayment.SelectedItem)}' and serviceType = 'communal'";
